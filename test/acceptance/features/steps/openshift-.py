...
class Openshift():  
    ...
    def create_new_app(self, app_name, project):
        ...
        cmd = "oc new-app {} --name {} -n {}".format(nodjs_app_arg,app_name,project)
    ...
    def get_build_status(self):
        ...
        cmd_build_status = 'oc get build {} -o "jsonpath={.status.phase}"'.format(build_name)
        build_status = self.cmdObj.run_check_for_status(cmd_build_status, "Complete", 5, 300)
        build_status | should_not.be_equal_to(None)
        return build_name, build_status

    def get_deployment_config(self, project):
        cmd = 'oc get dc -n {} -o "jsonpath={.items[*].metadata.name}"'.format(project)
        deployment_config = self.cmdObj.run(cmd)
        deployment_config | should_not.be_equal_to(None)
        return deployment_config 

    def delete_deployment_config(self, deployment_config, project):
    ...
    def use_deployment(self, application_name, deployment_config, project):
        self.delete_deployment_config(deployment_config, project) | should.be_truthy
